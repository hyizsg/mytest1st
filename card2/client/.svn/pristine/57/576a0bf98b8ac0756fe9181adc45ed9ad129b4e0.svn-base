...
void HpAnimRenderVisitor::end(){
    ...
    for(int i = m_chr_instance->getAtlases()->count() - 1; i > m_cur_atlas_id; -- i){
        CCTextureAtlas* atlas = dynamic_cast<CCTextureAtlas*> (m_chr_instance->getAtlases()->objectAtIndex(i));
...
void HpAnimRenderVisitor::visitImageKey(HpImageKeyframe *p_ikf, HpKeyframe *p_frm, float time){
    ...
    if(m_cur_atlas == NULL || m_cur_atlas->getTexture() != p_ikf->getSpriteFrame()->getTexture()){
        ...
        m_cur_atlas = static_cast<CCTextureAtlas*>(m_chr_instance->getAtlases()->objectAtIndex(m_cur_atlas_id));
    ...
    if(m_cur_atlas->getTexture() && m_cur_atlas->getTexture()->hasPremultipliedAlpha() && color.a != 1.0f){
        float a = color.a;
        color.a = 1.0f;
